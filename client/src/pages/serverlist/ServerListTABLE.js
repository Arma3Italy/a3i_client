    ...
    listItem() {
        const sl = this.state.servers.map(i => (
            <div className="ST_body row bg-light text-dark p-2 border-bottom" key={String(i.addr)}>
                ...
                <div className="col-6 col-lg-4 text-wrap">{this.serverStatusChecker(i)} <a href={`/server/${i.addr}`}><span className="badge badge-primary">{i.gametype === undefined ? 'NONE' : i.gametype}</span> {i.name}</a></div>
