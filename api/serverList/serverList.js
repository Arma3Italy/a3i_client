...
const mainParh = pathM.resolve(__dirname, '..', '..');
...
module.exports = function () {
    require('https').request({ hostname, path, method: 'GET' }, (res) => {
        ...
        res.on('end', async () => {
            ...
            fs.readdir(pathM.resolve(mainParh, 'db'), (err, data) => {
                if (err) {
                    fs.mkdir(pathM.resolve(mainParh, 'db'), (mkerr, mkdata) => {
                        fs.writeFileSync(pathM.resolve(mainParh, 'db', 'serverList.json'), JSON.stringify(serverList));
                    });
                } else {
                    fs.readFile(pathM.resolve(mainParh, 'db', 'serverList.json'), 'utf8', async (rferr, rfdata) => {
                        if (rferr) { fs.writeFileSync(pathM.resolve(mainParh, 'db', 'serverList.json'), JSON.stringify(serverList)) }
                        else {
                            ...
                            fs.writeFileSync(pathM.resolve(mainParh, 'db', 'serverList.json'), JSON.stringify(rfdata))
                        };
                    });
                }
            });
